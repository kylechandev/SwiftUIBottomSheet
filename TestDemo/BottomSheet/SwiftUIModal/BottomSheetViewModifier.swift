...
private struct BottomSheetViewModifier<SheetContent>: ViewModifier where SheetContent: View {
    ...
    func body(content: Content) -> some View {
        content.background( // 必须用`background`，如果用`overlay`的话会导致页面无法响应
...
public extension View {
    ...
    func bottomSheetIOS15Custom<Content>(
        isPresented: Binding<Bool>,
        @ViewBuilder content: @escaping () -> Content
    ) -> some View where Content: View {
        modifier(BottomSheetViewModifier(isPresented: isPresented, content: content))
    }
    ...
    func bottomSheetIOS15Custom<Item, Content>(
        item: Binding<Item?>,
        @ViewBuilder content: @escaping (Item) -> Content
    ) -> some View where Content: View {
        bottomSheetIOS15Custom(isPresented: item.isPresent(), content: {
